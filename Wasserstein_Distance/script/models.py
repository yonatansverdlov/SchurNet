...
class SharedProductNet(nn.Module):
    def __init__(self, encoder_params: dict, phi_params: dict, rho_params: dict, activation: str, bn: bool,
                 slope: float):
        super(SharedProductNet, self).__init__()
        self.encoder = SharedMLP(encoder_params['input_dim'], encoder_params['hidden'], encoder_params['output'],
                                 encoder_params['layers'], activation=activation, batch_norm=bn, slope=slope)
        self.phi = initialize_mlp(encoder_params['output'], phi_params['hidden'], phi_params['output'],
                                  phi_params['layers'], activation=activation, batch_norm=False, slope=slope)
        self.rho = initialize_mlp(phi_params['output'], rho_params['hidden'], 1, rho_params['layers'],
                                  activation=activation, batch_norm=False, slope=slope)
